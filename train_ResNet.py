...
def run_train():
    ...
    train_writer = tf.summary.FileWriter( './outputs/tensorboard/6',
                                      sess.graph)
    with sess.as_default():
        ...
        var_lt=[v for v in tf.trainable_variables() if v.name.startswith('res')]#resnet_v1_50
        # pdb.set_trace()
    ...
    #                 cv2.waitKey(0)
            ...
            if (iter)%2000==0 and (iter!=0):
                ...
                saver.save(sess, out_dir + '/check_points/snap_ResNet50_NGT_%06d.ckpt'%iter)  #iter
