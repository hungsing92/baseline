...
def run_train():
    ...
    if 1:
        ...
        bases=np.array([[-19.5, -0.8, 19.5, 0.8],
                        [-0.8, -19.5, 0.8, 19.5],
                        [-0.5, -0.3, 0.5, 0.3],
                        [-0.3, -0.5, 0.3, 0.5]
                        ])
        ...
        rgbs, tops, fronts, gt_labels, gt_boxes3d, top_imgs, front_imgs, rgbs_norm, image_index = load_dummy_datas(index[:1])
    ...
    solver_step = solver.minimize(1*top_cls_loss+0.5*top_reg_loss+3*fuse_cls_loss+0.5*fuse_reg_loss+l2)
    ...
    with sess.as_default():
        ...
        saver.restore(sess, './outputs/check_points/snap_065000.ckpt') 
        # saver.restore(sess, './outputs/check_points/MobileNet.ckpt')  
        # var_lt=[v for v in tf.trainable_variables() if v.name.startswith('MobileNet')]
        # # var_lt.pop(0)
        # # var_lt.pop(0)
        # saver_0=tf.train.Saver(var_lt)        
        # # 
        # saver_0.restore(sess, '/home/hhs/4T/hongsheng/pretrained model/mobilenet_width_1.0_preprocessing_same_as_inception/model.ckpt')
        # # pdb.set_trace()
        ...
        for iter in range(max_iter):
            ...
            #shuffle the samples every 2*num_frames
            ...
            #load 1000 samples every 2000 iterations
            if idx%1000==0 :
                count+=idx
                if count%2000==0:
                    frame+=idx
                    frame_end=min(frame+1000,num_frames)
            ...
            # # pdb.set_trace()
            # inside_inds_filtered=anchor_filter(top_img[:,:,0], anchors, inside_inds)
            ...
            if (iter)%5000==0 and (iter!=0):
                ...
                saver.save(sess, out_dir + '/check_points/snap_%06d.ckpt'%iter)  #iter
