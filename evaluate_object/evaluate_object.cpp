...
const int32_t N_TESTIMAGES = 1900;
...
void saveStats (const vector<double> &precision, const vector<double> &recall, const vector<double> &aos, FILE *fp_det, FILE *recall_det, FILE *fp_ori) {
  ...
  if(recall.empty())
  	return;
  for (int32_t i=0; i<recall.size(); i++)
  	fprintf(recall_det,"%f ",recall[i]);
  fprintf(recall_det, "\n");
...
bool eval_class (FILE *fp_det, FILE *recall_det, FILE *fp_ori, CLASSES current_class,const vector< vector<tGroundtruth> > &groundtruth,const vector< vector<tDetection> > &detections, bool compute_aos, vector<double> &precision, vector<double> &aos, DIFFICULTY difficulty) {
  ...
  saveStats(precision, recall, aos, fp_det, recall_det, fp_ori);
	return true;
}
...
bool eval(string path, string path_to_gt, Mail* mail){
  ...
  FILE *fp_det=0, *fp_ori=0, *recall_det=0;

  // eval cars
  if(eval_car){
    fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[CAR] + "_detection.txt").c_str(),"w");
    recall_det = fopen((result_dir + "/stats_" + CLASS_NAMES[CAR] + "_recall.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[CAR] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if(   !eval_class(fp_det,recall_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[0],aos[0],EASY)
       || !eval_class(fp_det,recall_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE)
       || !eval_class(fp_det,recall_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[2],aos[2],HARD)){
  ...
  if(eval_pedestrian){
    ...
    recall_det = fopen((result_dir + "/stats_" + CLASS_NAMES[PEDESTRIAN] + "_recall.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[PEDESTRIAN] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if(   !eval_class(fp_det,recall_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[0],aos[0],EASY)
       || !eval_class(fp_det,recall_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE)
       || !eval_class(fp_det,recall_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[2],aos[2],HARD)){
  ...
  if(eval_cyclist){
    ...
    recall_det = fopen((result_dir + "/stats_" + CLASS_NAMES[CYCLIST]  + "_recall.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[CYCLIST] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if(   !eval_class(fp_det,recall_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[0],aos[0],EASY)
       || !eval_class(fp_det,recall_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE)
       || !eval_class(fp_det,recall_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[2],aos[2],HARD)){
